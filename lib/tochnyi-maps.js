(function(root, factory) {
    ...
    function visualBoundsFromColumnWeights(weights, viewportWidth, trimRatio) {
        ...
        if (!values.length || !total || !Number.isFinite(viewportWidth) || viewportWidth <= 0) return null;
        var trim = clamp(trimRatio === undefined ? 0.005 : trimRatio, 0, 0.2);
        var leftTarget = total * trim;
        var rightTarget = total * trim;
        var cumulative = 0;
        var leftIndex = 0;
        var rightIndex = values.length - 1;
        for (var index = 0; index < values.length; index += 1) {
            cumulative += Math.max(0, Number(values[index]) || 0);
            if (cumulative >= leftTarget) {
                leftIndex = index;
                break;
            }
        }
        ...
        for (var reverse = values.length - 1; reverse >= 0; reverse -= 1) {
            ...
            if (cumulative >= rightTarget) {
                rightIndex = reverse;
                break;
            }
        }
        var left = leftIndex / values.length * viewportWidth;
        var right = (rightIndex + 1) / values.length * viewportWidth;
        return {
            left: left,
            right: right,
            width: right - left,
