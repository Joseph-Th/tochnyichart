(function(root, factory) {
    ...
    function roundedGridCandidate(points, cardX, requestedRadius) {
        ...
        for (var index = 1; index < points.length - 1; index += 1) {
            ...
            var outgoingDirection = next.x - corner.x;
            var terminalCardDirection = Number(cardX) - next.x;
            if (index === points.length - 2 &&
                Math.abs(corner.y - next.y) < 0.5 &&
                Math.abs(outgoingDirection) > 0.5 &&
                outgoingDirection * terminalCardDirection < -0.5) {
                if (Math.hypot(corner.x - cursor.x, corner.y - cursor.y) > 0.5) {
                    path.push('L', corner.x, corner.y);
                    segments.push(straightCubicSegment(cursor, corner));
                }
                path.push('H', cardX);
                segments.push(straightCubicSegment(corner, { x: cardX, y: corner.y }));
        ...
        var approachDirection = end.x - cursor.x;
        var cardDirection = Number(cardX) - end.x;
        var terminalBacktrack = Math.abs(cursor.y - end.y) < 0.5 &&
            Math.abs(approachDirection) > 0.5 &&
            approachDirection * cardDirection < -0.5;
        if (terminalBacktrack) {
            // A detour can arrive at the port from the card side, then the
            // terminal H segment would reverse over the final few pixels.
            // The port is only a routing target; the visible leader should
            // finish from the detour cursor directly into the card.
            var repairedEnd = { x: Number(cardX), y: cursor.y };
            path.push('H', repairedEnd.x);
            segments.push(straightCubicSegment(cursor, repairedEnd));
            return { path: path.join(' '), segments: segments };
        }
