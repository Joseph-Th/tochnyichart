(function(root, factory) {
    ...
    function distributeLeaderLanes(entries, top, bottom, requestedGap) {
        ...
        var gap = ordered.length > 1
            ? Math.min(requestedGap, available / (ordered.length - 1))
            : 0;
        var positions = ordered.map(function(entry) {
            return clamp(entry.point.y, top, bottom);
        });
        for (var index = 1; index < positions.length; index += 1) {
            positions[index] = Math.max(positions[index], positions[index - 1] + gap);
        }
        if (positions[positions.length - 1] > bottom) {
            positions[positions.length - 1] = bottom;
            for (var reverse = positions.length - 2; reverse >= 0; reverse -= 1) {
                positions[reverse] = Math.min(positions[reverse], positions[reverse + 1] - gap);
            }
        }
        if (positions[0] < top) {
            var shift = top - positions[0];
            positions = positions.map(function(position) { return position + shift; });
        }
        ordered.forEach(function(entry, laneIndex) {
            entry.routeY = positions[laneIndex];
    ...
    function planLeaderRoutes(entries, options) {
        ...
        ['left', 'right'].forEach(function(side) {
            ...
            sideEntries.forEach(function(entry) { entry.sideCount = sideEntries.length; });
            if (sideRouting === 'lanes') {
                distributeLeaderLanes(sideEntries, settings.top, settings.bottom, settings.gap || 16);
            } else {
                sideEntries.sort(function(first, second) { return first.point.y - second.point.y; });
                sideEntries.forEach(function(entry, laneIndex) {
                    entry.routeY = clamp(entry.point.y, settings.top, settings.bottom);
                    entry.laneIndex = laneIndex;
                });
            }
        });
    ...
    function buildOrthogonalLeaderPath(entry, geometry) {
        ...
        var routeY = Number(entry && entry.routeY);
        if (!Number.isFinite(routeY)) routeY = Number(point.y) || 0;
        var laneIndex = Math.max(0, Number(entry && entry.laneIndex) || 0);
        var sideCount = Math.max(1, Number(entry && entry.sideCount) || 1);
        var trackSpread = Number(settings.trackSpread) || 30;
        var approachSpread = Number(settings.approachSpread) || 42;
        var trackSpacing = sideCount > 1 ? Math.min(6, trackSpread / (sideCount - 1)) : 0;
        var approachSpacing = sideCount > 1 ? Math.min(9, approachSpread / (sideCount - 1)) : 0;
        var trackX = mapEdgeX + direction * (8 + laneIndex * trackSpacing);
        var desiredApproachX = mapEdgeX + direction * (42 + laneIndex * approachSpacing);
