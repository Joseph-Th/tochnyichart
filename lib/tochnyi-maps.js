(function(root, factory) {
    ...
    function planRegionalBreakdown(entries, options) {
        ...
        if (usePortRouting) {
            ...
            if (primaryPlacement.predictedCrossings > 0) {
            ...
            placement = balanceCalloutSides(all, placementOptions);
        }
    ...
    function buildPortLeaderPath(entry, geometry) {
        ...
        if (pointFacesCard && pointInsideCardBand && nearCardDistance <= nearCardThreshold) {
            ...
            if (!nearCardEvaluation.collisionCount && !nearCardCrossings) {
                return {
                    path: nearCard.path,
        ...
        if (!directEvaluation.collisionCount && !directRouteCrossings) return baseResult;
        ...
        if (!blockers.length && directRouteCrossings) {
        ...
        if (directRouteCrossings) {
            avoidRoutes.forEach(function(route) {
                if (!routeSegmentsConflict(directRouteSegments, route, 8)) return;
                var bounds = routeBounds(route);
                if (!bounds) return;
    ...
    function buildOrthogonalLeaderPath(entry, geometry) {
        ...
        var effectiveRouteY = Math.abs(routeY - Number(settings.endY || 0)) <= laneTolerance
            ...
            : routeY;
        var useMeaningfulFanOut = useFanOut && sourceLaneOffset > laneTolerance;
        ...
        var waypoints = useMeaningfulFanOut
            ? [
                { x: fanX, y: effectiveRouteY },
                { x: approachX, y: effectiveRouteY },
                { x: approachX, y: endY }
            ]
            : [{ x: approachX, y: endY }];
        var smooth = detourPortCandidate(
            point,
            waypoints,
            approachX,
            endY,
    ...
    function resolveLandmassPlan(map, regionSet, data, features) {
        ...
        var requested = regionSet && regionSet.requireFullContext
            ? 'all'
            : configuration.landmass || 'auto';
        var activeIds = [];
