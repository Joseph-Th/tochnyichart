(function(root, factory) {
    ...
    var regionalBreakdownPolicy = Object.freeze({
        // Port routing is now opt-in. The default regional map should read like
        // a human-arranged annotated map: short direct leaders to nearby boxes,
        // not a routing diagram optimized around every possible crossing.
        portRoutingThreshold: Infinity,
        denseThreshold: 9,
        compactColumnThreshold: 5,
        // Keep callout boxes measurably inside the clipped stage rather than
        // sitting exactly on the diagnostic boundary. Font metrics can settle
        // a fraction of a pixel differently across fresh headless profiles;
        // a real inset makes export containment deterministic.
        cardInset: 16,
        cardWidthDense: 190,
        ...
        sideSwitchToleranceRatio: 0.12,
        sideSwitchToleranceMinimum: 96,
        summaryRightGap: 22,
        summaryBelowGap: 18,
        legendGap: 8,
        stageInset: 16,
    ...
    function getRegionalBreakdownPolicy(options) {
        ...
        return {
            ...
            compactColumnThreshold: regionalBreakdownPolicy.compactColumnThreshold,
            ...
            sideSwitchToleranceRatio: regionalBreakdownPolicy.sideSwitchToleranceRatio,
            sideSwitchToleranceMinimum: regionalBreakdownPolicy.sideSwitchToleranceMinimum,
    ...
    function shouldRenderCallout(item, map) {
        if (map && map.callouts === 'none') return false;
        return !item || item.callout !== 'none';
    }

    function resolveCalloutSide(item, point, stageWidth) {
        var requested = item && item.calloutSide;
        if (requested === 'left' || requested === 'right') return requested;
        var x = Number(point && point.x);
        var width = Number(stageWidth);
        if (!Number.isFinite(x) || !Number.isFinite(width) || width <= 0) return 'left';
        return x < width / 2 ? 'left' : 'right';
    }

    function geographicSidePreference(entry, options) {
        var settings = options || {};
        var requested = entry && entry.item && entry.item.calloutSide;
        if (requested === 'left' || requested === 'right') {
            return { side: requested, locked: true, distanceAdvantage: Infinity };
        }
        var width = Number(settings.width) || 0;
        var x = Number(entry && entry.point && entry.point.x);
        if (!Number.isFinite(x) || width <= 0) {
            return { side: entry && entry.initialSide === 'right' ? 'right' : 'left', locked: false, distanceAdvantage: 0 };
        }
        var cardWidth = Number(settings.cardWidth) || 0;
        var cardInset = Number(settings.cardInset) || 0;
        var leftEndpoint = cardInset + cardWidth;
        var rightEndpoint = width - cardInset - cardWidth;
        var leftDistance = Math.abs(x - leftEndpoint);
        var rightDistance = Math.abs(rightEndpoint - x);
        var side = leftDistance <= rightDistance ? 'left' : 'right';
        var advantage = Math.abs(leftDistance - rightDistance);
        var tolerance = Math.max(
            Number(settings.sideSwitchToleranceMinimum) || regionalBreakdownPolicy.sideSwitchToleranceMinimum,
            width * (Number(settings.sideSwitchToleranceRatio) || regionalBreakdownPolicy.sideSwitchToleranceRatio)
        );
        return {
            side: side,
            locked: advantage > tolerance,
            distanceAdvantage: advantage
        };
    }

    function resolveAnchorStyle() {
        // Anchor dots at line origins add clutter and were never part of the
        // intended public design. Preserve the field for backward-compatible
        // specs, but render no dot even when an older spec requests one.
        return 'none';
    }

    function simpleCalloutPlacement(entries, options) {
        var settings = options || {};
        ...
        all.forEach(function(entry, index) {
            entry._placementIndex = Number.isFinite(Number(entry._placementIndex))
                ? Number(entry._placementIndex)
                : index;
            entry.initialSide = entry.side === 'right' ? 'right' : 'left';
            var preference = geographicSidePreference(entry, settings);
            entry.side = preference.side === 'right' ? 'right' : 'left';
            if (entry.side === 'right') right.push(entry);
            else left.push(entry);
        });

        function orderColumn(column) {
            column.sort(function(first, second) {
                var vertical = Number(first.point && first.point.y) - Number(second.point && second.point.y);
                if (Math.abs(vertical) > 0.5) return vertical;
                var horizontal = Number(first.point && first.point.x) - Number(second.point && second.point.x);
                if (Math.abs(horizontal) > 0.5) return horizontal;
                return first._placementIndex - second._placementIndex;
            });
        }

        orderColumn(left);
        orderColumn(right);
        // Default callout order is geometry-owned. Reorder the already chosen
        // side columns to minimize straight-line crossings and total travel;
        // source/data order and calloutOrder are not routing instructions.
        var optimized = optimizeColumnOrder(left, right, settings);
        left = optimized.left;
        right = optimized.right;
        left.forEach(function(entry, index) { entry.placementOrder = index; });
        right.forEach(function(entry, index) { entry.placementOrder = index; });
        var metrics = optimized.metrics || scoreCalloutPlacement(left, right, settings);
        return {
            left: left,
            right: right,
            predictedCrossings: metrics.crossings || 0,
            predictedCrowding: metrics.crowding || 0,
            predictedLength: metrics.length || 0,
            maximumAttachmentSlope: metrics.maximumAttachmentSlope || 0,
            attachmentSharpness: metrics.attachmentSharpness || 0,
            sideSwitches: 0,
            geographicSideLocks: 0,
    ...
    function resolveCalloutDistribution(map, summaryShown, sideEntries) {
        ...
        // Keep legacy `balanced` specs parseable, but do not spread boxes just
        // to fill a column. Geographic packing keeps cards near their anchors.
        if (requested === 'geographic' || requested === 'balanced') return 'geographic';
        return 'geographic';
    }
    ...
    function resolveLeaderRouting(map) {
        var requested = map && map.leaderRouting || 'auto';
        if (requested === 'direct' || requested === 'ports' || requested === 'indexed') return requested;
        // Automatic routing is intentionally boring: one direct leader per
        // callout. More elaborate modes are explicit exceptions, never a
        // heuristic response to local crowding.
        return 'direct';
    }
    ...
    function sampledPlacementRoute(route, options) {
        return [
            { x: Number(route.start.x), y: Number(route.start.y) },
            { x: Number(route.end.x), y: Number(route.end.y) }
        ];
    }
    ...
    function scoreCalloutPlacement(leftEntries, rightEntries, options) {
        ...
        var sideStabilityPenalty = switches * 5000000;
        return {
            crossings: crossings,
            crowding: 0,
    ...
    function optimizeCalloutPlacement(entries, options) {
        ...
        all.forEach(function(entry, index) {
            ...
            entry.geographicSidePreference = geographicSidePreference(entry, settings);
        });
        var fixedLeft = all.filter(function(entry) {
            return entry.geographicSidePreference.locked && entry.geographicSidePreference.side === 'left';
        });
        var fixedRight = all.filter(function(entry) {
            return entry.geographicSidePreference.locked && entry.geographicSidePreference.side === 'right';
        });
        var automatic = all.filter(function(entry) {
            return !entry.geographicSidePreference.locked;
        });
        ...
        return {
            ...
            geographicSideLocks: fixedLeft.length + fixedRight.length,
    ...
    function planRegionalBreakdown(entries, options) {
        ...
        var placementOptions = {
            ...
            sideSwitchToleranceRatio: policy.sideSwitchToleranceRatio,
            sideSwitchToleranceMinimum: policy.sideSwitchToleranceMinimum
        };
        var placement;
        if (usePortRouting) {
            ...
            // Default maps keep cards on their geographically sensible side,
            // then optimize vertical order for straight, non-crossing leaders.
            placement = simpleCalloutPlacement(all, placementOptions);
        }
        ...
        return {
            ...
            placementMode: usePortRouting ? 'crossing-optimized-ports' : 'direct-optimized',
            placement: placement,
    ...
    function buildOrthogonalLeaderPath(entry, geometry) {
        ...
        var routeY = Number.isFinite(Number(entry && entry.routeY))
            ? Number(entry.routeY)
            : Number(point.y) || 0;
        var start = { x: Number(point.x) || 0, y: Number(point.y) || 0 };
        var end = { x: cardX, y: endY };

        return {
            path: ['M', start.x, start.y, 'L', end.x, end.y].join(' '),
            routeSegments: [straightCubicSegment(start, end)],
            approachX: cardX,
            fanX: Number(point.x),
            routeY: routeY,
            endY: endY,
    ...
    return {
        ...
        shouldRenderCallout: shouldRenderCallout,
        resolveCalloutSide: resolveCalloutSide,
        geographicSidePreference: geographicSidePreference,
