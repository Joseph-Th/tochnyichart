(function(root, factory) {
    ...
    function simpleCalloutPlacement(entries, options) {
        ...
        function orderColumn(column) {
            column.sort(function(first, second) {
                var firstOrder = first.item && first.item.calloutOrder;
                var secondOrder = second.item && second.item.calloutOrder;
                if (firstOrder !== undefined || secondOrder !== undefined) {
                    return (firstOrder === undefined ? 999 : firstOrder) -
                        (secondOrder === undefined ? 999 : secondOrder);
                }
                return first._placementIndex - second._placementIndex;
            });
            column.forEach(function(entry, index) {
        ...
        return {
            left: left,
            right: right,
            predictedCrossings: 0,
            predictedCrowding: 0,
            predictedLength: 0,
            maximumAttachmentSlope: 0,
            attachmentSharpness: 0,
            sideSwitches: 0,
    ...
    function planRegionalBreakdown(entries, options) {
        ...
        if (usePortRouting) {
            ...
            // Default maps use a simple editorial layout. Place cards on the
            // closest side and preserve explicit calloutOrder or data order
            // within each column. This favors predictable reading and short
            // human-like leaders over over-optimized crossing math.
        ...
        return {
            ...
            placementMode: usePortRouting ? 'crossing-optimized-ports' : 'direct-editorial',
            placement: placement,
    ...
    function buildOrthogonalLeaderPath(entry, geometry) {
        ...
        var routeY = Number.isFinite(Number(entry && entry.routeY))
            ? Number(entry.routeY)
            : Number(point.y) || 0;
        // Regional leaders should read as plain annotations, not as a routing
        // diagram. Use the shortest smooth path to the card edge and avoid
        // routing corridors or detours in the default path.
        var cardStub = Math.max(18, Number(settings.fanStub) || 22);
        var approachX = cardX - direction * cardStub;
        if (side === 'left') {
