(function(root, factory) {
    ...
    function buildPortLeaderPath(entry, geometry) {
        ...
        var directRouteInterference = directRouteCrossings || directRouteCrowding.significant;
        ...
        if (avoidRoutes.length) {
            ...
            var noCrowdingRoutes = crowdingPool.filter(function(candidate) {
                return (candidate.routeCrossings || 0) === 0 &&
                    !(candidate.routeCrowding && candidate.routeCrowding.significant);
    ...
    function buildOrthogonalLeaderPath(entry, geometry) {
        ...
        var approachSpacing = sideCount > 1
            ? (Number.isFinite(requestedApproachSpacing)
                ...
                : Math.min(desiredApproachSpacing, approachSpread / (sideCount - 1)))
            : 0;
        var desiredApproachX = mapEdgeX + direction * (26 + approachLaneIndex * approachSpacing);
        ...
        if (useFanOut) {
            ...
            var requestedFanStub = Number(settings.fanStub) || 18;
            var staggeredFanStub = requestedFanStub + laneIndex * 6;
            var fanStub = Math.min(
                staggeredFanStub,
                Math.max(8, availableFanSpan * 0.42)
            );
            fanX = Number(point.x) + (side === 'left' ? -fanStub : fanStub);
        }

        // A lane offset that is only a few pixels from the source is not a
        // meaningful routing constraint. Preserving it creates a tiny
        // back-and-forth spline before the leader heads to the card, which
        // reads as a sharp angle at small export sizes. Likewise, a tiny
        // final vertical leg is visually indistinguishable from the card
        // attachment and should not become its own spline section.
        var laneTolerance = Math.max(
            8,
            Math.min(14, Number(entry && entry.routeGap) * 0.45 || 9)
        );
        var sourceLaneOffset = Math.abs(routeY - Number(point.y));
        var destinationOffset = Math.abs(endY - Number(point.y));
        var effectiveRouteY = Math.abs(routeY - Number(settings.endY || 0)) <= laneTolerance
            ? Number(settings.endY || 0)
            : routeY;
        var useMeaningfulFanOut = useFanOut && (
            sourceLaneOffset > laneTolerance ||
            destinationOffset > laneTolerance * 2
        );

        // Keep the lane geometry as a set of waypoints for separation, but
        // draw it with tangent-continuous cubic sections. The old H/V path
        // made every lane turn look like a square elbow, especially when a
        // synthetic sample had only a few regions and used direct routing.
        var smooth;
        if (useMeaningfulFanOut) {
            var start = { x: Number(point.x), y: Number(point.y) };
            var fan = { x: fanX, y: effectiveRouteY };
            var end = { x: approachX, y: endY };
            var travelDirection = side === 'left' ? -1 : 1;
            var first = straightCubicSegment(start, fan);
            var horizontalSpan = Math.max(1, Math.abs(end.x - fan.x));
            var transitionCorridor = Math.max(
                3,
                Math.min(10, approachSpacing - leaderClearance - 2)
            );
            var transitionSpan = Math.min(
                horizontalSpan * 0.48,
                transitionCorridor
            );
            var bend = {
                x: end.x - travelDirection * transitionSpan,
                y: fan.y
            };
            var second = straightCubicSegment(fan, bend);
            var transitionHandle = Math.max(3, transitionSpan * 0.38);
            var third = {
                start: bend,
                control1: {
                    x: bend.x + travelDirection * transitionHandle,
                    y: bend.y
                },
                control2: {
                    x: end.x - travelDirection * transitionHandle,
                    y: end.y
                },
                end: end
            };
            smooth = {
                path: [
                    'M', start.x, start.y,
                    'C', first.control1.x, first.control1.y,
                    first.control2.x, first.control2.y,
                    first.end.x, first.end.y,
                    'C', second.control1.x, second.control1.y,
                    second.control2.x, second.control2.y,
                    second.end.x, second.end.y,
                    'C', third.control1.x, third.control1.y,
                    third.control2.x, third.control2.y,
                    third.end.x, third.end.y,
                    'H', cardX
                ].join(' '),
                segments: [first, second, third]
            };
        } else {
            smooth = detourPortCandidate(
                point,
                [{ x: approachX, y: endY }],
                approachX,
                endY,
