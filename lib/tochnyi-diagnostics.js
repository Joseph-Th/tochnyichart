(function(root, factory) {
    ...
    function collectDom(main) {
        var labels = [];
        var objects = [];
        var boundaries = [{ source: 'dom', rect: normalizeRect(main.getBoundingClientRect()) }];
    ...
    function diagnoseBoxes(input, options) {
        ...
        labels.forEach(function(label) {
            ...
            var boundary = boundaries.find(function(item) {
                return item.source === label.source;
            }) || boundaries[0];
            if (!boundary) return;
        ...
        for (var i = 0; i < labels.length; i += 1) {
            for (var j = i + 1; j < labels.length; j += 1) {
                var first = labels[i];
                var second = labels[j];
                var overlap = intersectionRatio(first.rect, second.rect);
    ...
    function run(options) {
        ...
        var issues = diagnoseBoxes({
            ...
            boundaries: boundaries
        }, options).concat(diagnoseMarkStyles(objects, options));
