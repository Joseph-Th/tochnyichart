(function(root, factory) {
                    ...
                    try {
                        if (sprite.isVisibleDeep()) {
                            objects.push({
                                ...
                                source: 'amcharts',
                                role: cls === 'Grid' ? 'reference-line' : 'column',
                                dataUid: sprite.dataItem ? sprite.dataItem.uid : null,
        ...
        elements.forEach(function(element, index) {
            ...
            var role = element.getAttribute('data-label-role') || (
                element.classList.contains('tochnyi-change-badge')
                    ? 'emphasis-badge'
                    : element.classList.contains('tochnyi-stat-value')
                        ? 'stat-value'
                        : element.classList.contains('tochnyi-stat-label')
                            ? 'stat-label'
    ...
    function diagnoseBoxes(input, options) {
        ...
        labels.forEach(function(label) {
            objects.forEach(function(object) {
                ...
                if (label.dataUid && object.dataUid && label.dataUid === object.dataUid) return;
                if (label.group && object.group && label.group === object.group) return;

                if (object.line) {
                    if (label.role.indexOf('axis-label') >= 0) return;
                    if (lineHitsRect(object, label.rect, config.lineTolerance)) {
                        issues.push(makeIssue(
                            'text-line-collision',
                            'warning',
    ...
    function diagnoseViewportFit(main, options) {
        ...
        var bottomPadding = parseFloat(style.paddingBottom) || 0;
        var mainRect = normalizeRect(main.getBoundingClientRect());
        var capture = requestedCaptureSize();
        var viewport = normalizeRect({
            left: 0,
            top: 0,
            right: Math.max(0, capture.width - rightPadding),
            bottom: Math.max(0, capture.height - bottomPadding)
        });
        var verticalOverflow = Math.max(0, mainRect.bottom + bottomPadding - capture.height);
        var horizontalOverflow = Math.max(0, mainRect.right + rightPadding - capture.width);
    ...
    function run(options) {
        ...
        return writeReport({
            ...
            summary: {
                ...
                marksChecked: objects.filter(function(object) { return object.role === 'column'; }).length,
                errors: errors,
                warnings: warnings
            },
            issues: issues
