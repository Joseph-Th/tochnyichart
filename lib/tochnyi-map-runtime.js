(function(global) {
    ...
    function rasterizedVisualBounds(features, convert, viewportWidth, viewportHeight) {
        ...
        var weights = new Array(rasterWidth).fill(0);
        for (var y = 0; y < rasterHeight; y += 1) {
            for (var x = 0; x < rasterWidth; x += 1) {
                if (pixels[(y * rasterWidth + x) * 4 + 3] > 16) weights[x] += 1;
            }
        }
        return global.TochnyiMaps.visualBoundsFromColumnWeights(weights, viewportWidth, 0.0075);
    }
    ...
    function render(spec, chartNode) {
        ...
        function pixelsPerLongitude(bounds) {
            if (!bounds) return 0;
            var centerLongitude = (bounds.left + bounds.right) / 2;
            var centerLatitude = (bounds.bottom + bounds.top) / 2;
            var first = chart.convert({ longitude: centerLongitude - 0.5, latitude: centerLatitude });
            var second = chart.convert({ longitude: centerLongitude + 0.5, latitude: centerLatitude });
            if (!first || !second || !Number.isFinite(first.x) || !Number.isFinite(second.x)) return 0;
        ...
        function applyVisualCentering() {
            ...
            var correction = global.TochnyiMaps.correctViewportCenter(
                currentGeoBounds,
                projected,
                canvasWidth,
                pixelsPerLongitude(currentGeoBounds),
                { tolerance: 1.5, maximumShiftRatio: 0.22 }
            );
            ...
            chartNode.setAttribute('data-map-center-pixel-shift', String(Math.round(correction.pixelShift * 10) / 10));
            if (correction.centered || !correction.longitudeShift) return false;
            currentGeoBounds = correction.geoBounds;
            mapPlan.centerShiftLongitude += correction.longitudeShift;
            chartNode.setAttribute('data-map-center-shift-longitude', String(Math.round(mapPlan.centerShiftLongitude * 1000) / 1000));
        ...
        function settleViewport() {
            if (centerPass >= 3) return;
            centerPass += 1;
            if (applyVisualCentering()) {
                setTimeout(settleViewport, 45);
            }
        }
        ...
        polygonSeries.events.on('datavalidated', function() {
            applyViewport();
            setTimeout(settleViewport, 35);
            setTimeout(layout, 90);
