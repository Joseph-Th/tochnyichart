(function(global) {
    ...
    function balanceSides(entries, summaryOnRight, width) {
    ...
    function render(spec, chartNode) {
        ...
        function layout() {
            ...
            var requestedRouting = spec.map.leaderRouting || 'auto';
            var usePortRouting = requestedRouting === 'ports' || (
                requestedRouting === 'auto' && entries.length >= 8
            );
            ...
            var desiredLeft = Math.ceil(entries.length * (summaryOnRight ? 0.58 : 0.5));
            var predictedLeftDistribution = global.TochnyiMaps.resolveCalloutDistribution(
                spec.map,
                Boolean(summary),
                entries.slice(0, desiredLeft)
            );
            var predictedRightDistribution = global.TochnyiMaps.resolveCalloutDistribution(
                spec.map,
                Boolean(summary),
                entries.slice(desiredLeft)
            );
            var placement = usePortRouting
                ? global.TochnyiMaps.optimizeCalloutPlacement(entries, {
                    width: width,
                    cardWidth: cardWidth,
                    cardInset: 10,
                    attachmentInset: chartNode.classList.contains('is-dense') ? 14 : 16,
                    topLeft: topLeft,
                    topRight: topRight,
                    bottom: bottom,
                    gap: gap,
                    ...
                    leftDistribution: predictedLeftDistribution,
                    rightDistribution: predictedRightDistribution
                })
                : balanceSides(entries, summaryOnRight, width);
            var sides = { left: placement.left, right: placement.right };
            chartNode.setAttribute(
                'data-map-callout-placement',
                usePortRouting ? 'crossing-optimized' : 'balanced-nearest-side'
            );
            if (usePortRouting) {
            ...
            sides.left.concat(sides.right).forEach(function(entry) {
                ...
                var attachmentInset = chartNode.classList.contains('is-dense') ? 14 : 16;
            ...
            var routedEntries = global.TochnyiMaps.planLeaderRoutes(sides.left.concat(sides.right), {
                ...
                top: routeTop,
                bottom: routeBottom,
                gap: chartNode.classList.contains('is-dense') ? 18 : 22
            });
            ...
            if (routedEntries.routing === 'ports') {
                ...
                var shapeClearance = chartNode.classList.contains('is-dense') ? 3.25 : 8;
                ...
                var portRenderPlan = routedEntries.map(function(entry) {
                    ...
                    var leaderPath = global.TochnyiMaps.buildPortLeaderPath(entry, {
                        mapEdgeX: mapEdgeX,
                        cardX: cardX,
                        cardTop: entry.top,
                        ...
                        endY: entry.endY,
                        portOffset: 10,
                        minimumCardStub: chartNode.classList.contains('is-dense') ? 36 : 32,
                        obstacles: obstacles,
                        sourceObstacles: sourceObstacles,
                        obstacleClearance: shapeClearance,
                        routeTop: routeTop,
                        routeBottom: routeBottom,
                        routeLeft: canvasRect.left - stageRect.left + 2,
                        routeRight: canvasRect.right - stageRect.left - 2,
            ...
            if (routedEntries.routing === 'indexed') {
                ...
                var markerPlan = global.TochnyiMaps.planIndexedMarkers(routedEntries, {
                    ...
                    markerRadius: chartNode.classList.contains('is-dense') ? 11 : 12,
                    minimumDistance: chartNode.classList.contains('is-dense') ? 28 : 30,
                    candidateStep: chartNode.classList.contains('is-dense') ? 24 : 26
                });
