(function(global) {
    ...
    function lockMapOrientation(node) {
        if (!node || !node.style) return;
        node.style.setProperty('transform', 'none', 'important');
        node.style.setProperty('rotate', 'none', 'important');
        node.style.setProperty('transform-origin', '50% 50%', 'important');
    }
    ...
    function resolveInteriorAnchor(feature, pathNode, projection, geometryLayer, viewport, preferred) {
        ...
        // The projected area centroid is the most faithful visual center when
        // it is inside the rendered region. Only pay the grid-search cost for
        // concave or multipart shapes whose centroid falls outside the fill.
        ...
        var candidates = [preferred, { x: bounds.centerX, y: bounds.centerY }];
        var columns = 12;
        var rows = 12;
        ...
        // When the centroid is outside a concavity, choose the closest viable
        // candidate with the strongest interior probe score. That keeps the
        // leader in the body of the region without pushing it to an edge just
        // to face a callout column.
    ...
    function buildCard(item, calloutIndex, range, dataIndex) {
        var color = itemColor(item, range);
        var card = element('article', 'tochnyi-map-callout');
        card.setAttribute('data-map-index', String(dataIndex === undefined ? calloutIndex : dataIndex));
        card.setAttribute('data-map-callout-index', String(calloutIndex));
        ...
        header.appendChild(element('span', 'tochnyi-map-index', String(calloutIndex + 1)));
    ...
    function render(spec, chartNode) {
        ...
        var isMatrixStage = chartNode.classList.contains('regional-routing-matrix-stage') ||
            Boolean(chartNode.parentElement && chartNode.parentElement.classList.contains('regional-routing-matrix-stage'));

        chartNode.classList.add('tochnyi-map-stage');
        chartNode.setAttribute('data-map-workflow', 'regional-breakdown');
        chartNode.setAttribute('data-map-orientation', 'north-up');
        chartNode.setAttribute('data-map-rotation', '0');
        chartNode.setAttribute('data-map-bearing', '0');
        chartNode.setAttribute('data-map-pitch', '0');
        chartNode.setAttribute('data-map-tilt', '0');
        lockMapOrientation(chartNode);
        var calloutData = spec.data.map(function(item, dataIndex) {
            return { item: item, dataIndex: dataIndex };
        }).filter(function(entry) {
            return global.TochnyiMaps.shouldRenderCallout(entry.item, spec.map);
        });
        var regionalPolicy = global.TochnyiMaps.getRegionalBreakdownPolicy({ count: calloutData.length });
        if (regionalPolicy.dense) chartNode.classList.add('is-dense');
        if (calloutData.length) chartNode.classList.add('has-callouts');
        chartNode.setAttribute('data-map-active-item-count', String(spec.data.length));
        chartNode.setAttribute('data-map-callout-count', String(calloutData.length));
        ...
        lockMapOrientation(mapCanvas);
        lockMapOrientation(lineLayer);
        ...
        var cards = calloutData.map(function(entry, calloutIndex) {
            var card = buildCard(entry.item, calloutIndex, range, entry.dataIndex);
            cardLayer.appendChild(card);
            return card;
        });
        var summaryPlan = isMatrixStage
            ? { show: false, mode: 'hide', reason: 'regional-matrix-inline-metadata' }
            : global.TochnyiMaps.resolveSummaryPlan(spec);
        ...
        if (landmassPlan.removedActiveRegionIds.length) {
            throw new Error(
                ...
                '. Detached regions and island fragments are intentionally unsupported by map.regional; use a separate standard chart or another non-map recipe.'
            );
        }
        var featureById = {};
        ...
        var centroids = calloutData.map(function(entry) {
            return featureCentroid(featureById[itemRegionIds(entry.item)[0]]);
        });
        ...
        var geometryLayer = svgElement('svg', {
            ...
            'data-map-orientation': 'north-up'
        });
        lockMapOrientation(geometryLayer);
        ...
        function renderGeometry() {
            ...
            projection = global.TochnyiMaps.buildStaticProjection(
                ...
                {
                    orientation: global.TochnyiMaps.mapOrientation,
            ...
            if (projection.rotation !== 0 || !projection.orientation ||
                projection.orientation.bearing !== 0 || projection.orientation.pitch !== 0 ||
                projection.orientation.tilt !== 0) {
                throw new Error('Regional map projection must remain permanently north-up.');
            }
            ...
            chartNode.setAttribute('data-map-projection-orientation', 'north-up');
            chartNode.setAttribute('data-map-projection-rotation', '0');
        ...
        function layout() {
            ...
            if (narrow || !cards.length) {
                ...
                chartNode.setAttribute('data-map-callout-predicted-crowding', '0');
                ...
                chartNode.setAttribute('data-map-port-crowding-score', '0');
                chartNode.setAttribute('data-map-port-crowded-routes', '0');
                chartNode.setAttribute('data-map-leader-crowding-score', '0');
                chartNode.setAttribute('data-map-leader-crowded-pairs', '0');
            ...
            var entries = cards.map(function(card, index) {
                ...
                var calloutEntry = calloutData[index];
                var item = calloutEntry.item;
                var regionId = itemRegionIds(item)[0];
                ...
                var side = global.TochnyiMaps.resolveCalloutSide(item, point, width);
                return {
                    card: card,
                    item: item,
                    index: index,
                    dataIndex: calloutEntry.dataIndex,
                    point: point,
                    side: side
                };
            });
            ...
            var matrixHeaderInset = isMatrixStage ? 160 : 0;
            var matrixFooterInset = isMatrixStage ? 34 : 0;
            var topLeft = regionalPolicy.stageInset + matrixHeaderInset;
            var topRight = summaryOnRight
                ? Math.ceil(summary.getBoundingClientRect().height) + regionalPolicy.summaryRightGap + matrixHeaderInset
                : regionalPolicy.stageInset + matrixHeaderInset;
            var bottom = height - Math.max(
                regionalPolicy.stageInset,
                regionalPolicy.calloutBottomInset || regionalPolicy.stageInset
            ) - matrixFooterInset;
            ...
            var initialLeftCount = entries.filter(function(entry) { return entry.side === 'left'; }).length;
            var initialRightCount = entries.length - initialLeftCount;
            if (!regionalPolicy.dense && Math.max(initialLeftCount, initialRightCount) >= regionalPolicy.compactColumnThreshold) {
                chartNode.classList.add('is-callout-compact');
                entries.forEach(function(entry) {
                    entry.height = Math.ceil(entry.card.getBoundingClientRect().height);
                });
            } else {
                chartNode.classList.remove('is-callout-compact');
            }
            ...
            chartNode.setAttribute(
                'data-map-callout-geographic-side-locks',
                String(placement.geographicSideLocks || 0)
            );
            if (usePortRouting) {
                chartNode.setAttribute('data-map-callout-predicted-crossings', String(placement.predictedCrossings));
                chartNode.setAttribute(
                    'data-map-callout-predicted-crowding',
                    String(Math.round((placement.predictedCrowding || 0) * 10) / 10)
                );
            ...
            pack(sides.left, topLeft, bottom, gap, usePortRouting ? 'optimized' : 'geographic');
            pack(sides.right, topRight, bottom, gap, usePortRouting ? 'optimized' : 'geographic');
            ...
            if (routedEntries.routing === 'ports') {
                ...
                function planPortRoutes() {
                    ...
                    var routeSettings = {
                        ...
                        leaderClearance: regionalPolicy.leaderClearance,
                        // Regional fills are evidence, not routing obstacles. The simple
                        // router only deconflicts against leaders already on this side.
                        simpleRouting: true,
                        routeTop: routeTop,
                    ...
                    var plan = {
                        ...
                        routeCrowding: Number(leaderPath.routeCrowding) || 0,
                        routeCrowdedLength: Number(leaderPath.routeCrowdedLength) || 0,
                        minimumRouteGap: Number.isFinite(Number(leaderPath.minimumRouteGap))
                            ? Number(leaderPath.minimumRouteGap)
                            : Infinity,
                        ...
                        directionReversalCount: Number(leaderPath.directionReversalCount) || 0,
                        reverseDistance: Number(leaderPath.reverseDistance) || 0,
                        controlReverseDistance: Number(leaderPath.controlReverseDistance) || 0,
                        terminalApproachRun: Number(leaderPath.terminalApproachRun) || 0,
                        terminalBoxTurn: Boolean(leaderPath.terminalBoxTurn)
                    };
                ...
                function routePlanMetrics(plan) {
                    return {
                        ...
                        crowding: Object.keys(plan.portPlansByIndex).reduce(function(total, index) {
                            return total + (Number(plan.portPlansByIndex[index].routeCrowding) || 0);
                        }, 0),
                ...
                function routeMetricsBetter(candidate, incumbent) {
                    ...
                    if (Math.abs(candidate.crowding - incumbent.crowding) > 0.5) {
                        return candidate.crowding < incumbent.crowding;
                    }
                ...
                if (primaryRouteMetrics.crossings || primaryRouteMetrics.crowding > 1) {
                ...
                chartNode.setAttribute('data-map-port-curve-model', 'single-cubic-port');
                ...
                var totalRouteCrowding = portRenderPlan.reduce(function(total, plan) {
                    return total + (Number(plan.routeCrowding) || 0);
                }, 0);
                var crowdedRouteCount = portRenderPlan.filter(function(plan) {
                    return (Number(plan.routeCrowdedLength) || 0) >= 18 &&
                        (Number(plan.routeCrowding) || 0) >= 8;
                }).length;
                var finiteRouteGaps = portRenderPlan.map(function(plan) {
                    return Number(plan.minimumRouteGap);
                }).filter(Number.isFinite);
                chartNode.setAttribute(
                    'data-map-port-crowding-score',
                    String(Math.round(totalRouteCrowding * 10) / 10)
                );
                chartNode.setAttribute('data-map-port-crowded-routes', String(crowdedRouteCount));
                chartNode.setAttribute(
                    'data-map-port-min-route-gap',
                    finiteRouteGaps.length
                        ? String(Math.round(Math.min.apply(null, finiteRouteGaps) * 10) / 10)
                        : 'none'
                );
                chartNode.setAttribute(
                ...
                chartNode.setAttribute('data-map-leader-crowded-pairs', String(crowdedRouteCount));
                chartNode.setAttribute(
                ...
                chartNode.setAttribute('data-map-port-directionality', 'monotonic-single-spline');
                ...
                chartNode.setAttribute(
                    'data-map-port-obstacle-avoidance',
                    'leader-lines-only'
                );
                chartNode.setAttribute('data-map-port-obstacle-count', '0');
                chartNode.setAttribute('data-map-port-avoided-obstacles', String(portRenderPlan.reduce(function(total, plan) {
                    return total + (Number(plan.avoidedObstacleCount) || 0);
                }, 0)));
                ...
                chartNode.setAttribute('data-map-port-direction-reversal-routes', String(portRenderPlan.filter(function(plan) {
                    return !plan.sourceExitUsed && (Number(plan.directionReversalCount) || 0) > 0;
                }).length));
                chartNode.setAttribute('data-map-port-control-reversal-routes', String(portRenderPlan.filter(function(plan) {
                    return !plan.sourceExitUsed && (Number(plan.controlReverseDistance) || 0) > 0.5;
                }).length));
                chartNode.setAttribute('data-map-port-terminal-box-turn-routes', String(portRenderPlan.filter(function(plan) {
                    return !plan.sourceExitUsed && plan.terminalBoxTurn;
                }).length));
                ...
                portRenderPlan.forEach(function(plan) {
                    ...
                    var leaderNode = svgElement('path', {
                        ...
                        'data-route-direction-reversals': String(plan.directionReversalCount || 0),
                        'data-route-reverse-distance': String(Math.round((plan.reverseDistance || 0) * 10) / 10),
                        'data-route-control-reverse-distance': String(Math.round((plan.controlReverseDistance || 0) * 10) / 10),
                        'data-route-terminal-approach-run': String(Math.round((plan.terminalApproachRun || 0) * 10) / 10),
                        'data-route-terminal-box-turn': plan.terminalBoxTurn ? 'true' : 'false',
            ...
            function buildSparseLeaderPlan(geometry) {
                return routedEntries.map(function(entry) {
                    var cardX = entry.side === 'left'
                        ? regionalPolicy.cardInset + cardWidth
                        : width - regionalPolicy.cardInset - cardWidth;
                    // Use the same attachment point that was already clamped to
                    // the card's usable vertical band. Falling back to a fixed
                    // top-biased offset can pull a leader away from its region
                    // and manufacture a large, needless curve.
                    var endY = Number.isFinite(Number(entry.endY))
                        ? Number(entry.endY)
                        : entry.top + Math.min(entry.height / 2, 33);
                    var mapEdgeX = entry.side === 'left'
                        ? canvasRect.left - stageRect.left
                        : canvasRect.right - stageRect.left;
                    var leaderPath = global.TochnyiMaps.buildOrthogonalLeaderPath(entry, {
                        ...
                        endY: endY,
                        leaderClearance: regionalPolicy.leaderClearance,
                        approachSpacing: geometry.approachSpacing,
                        fanStub: geometry.fanStub
                    });
                    return {
                        entry: entry,
                        pathData: leaderPath.path,
            ...
            function measureSparseLeaderPlan(plan) {
                var metrics = {
                    crowding: 0,
                    crowdedPairs: 0,
                    crossingPairs: 0,
                ...
                plan.forEach(function(item) {
                    item.crowdingScore = 0;
                    item.crowdedWith = [];
                    metrics.length += (item.routeSegments || []).reduce(function(total, segment) {
                        return total + Math.hypot(
                            Number(segment.end && segment.end.x) - Number(segment.start && segment.start.x),
                            Number(segment.end && segment.end.y) - Number(segment.start && segment.start.y)
                        );
                    }, 0);
                });
                for (var firstLeader = 0; firstLeader < plan.length; firstLeader += 1) {
                    for (var secondLeader = firstLeader + 1; secondLeader < plan.length; secondLeader += 1) {
                        var firstPlan = plan[firstLeader];
                        var secondPlan = plan[secondLeader];
                        // Opposite-side leaders naturally share a small source
                        // neighborhood before diverging. The routing decision is
                        // made per side, so only same-side corridors should
                        // compete for clearance.
                        if (firstPlan.entry.side !== secondPlan.entry.side) continue;
                        var pairCrowding = global.TochnyiMaps.routeSegmentSetCrowding(
                            firstPlan.routeSegments,
                            secondPlan.routeSegments,
                            regionalPolicy.leaderClearance
                        );
                        metrics.crowding += pairCrowding.score;
                        firstPlan.crowdingScore += pairCrowding.score;
                        secondPlan.crowdingScore += pairCrowding.score;
                        metrics.minimumGap = Math.min(metrics.minimumGap, pairCrowding.minimumDistance);
                        if (pairCrowding.minimumDistance <= 0.25) metrics.crossingPairs += 1;
                        if (pairCrowding.significant) {
                            metrics.crowdedPairs += 1;
                            firstPlan.crowdedWith.push(secondPlan.entry.index);
                            secondPlan.crowdedWith.push(firstPlan.entry.index);
                        }
                    }
                }
                return metrics;
            ...
            var baseApproachSpacing = Math.max(18, regionalPolicy.leaderClearance + 6);
            var approachSpacings = [
                baseApproachSpacing,
                baseApproachSpacing + 4,
                baseApproachSpacing + 8,
                baseApproachSpacing + 12,
                baseApproachSpacing + 16
            ];
            var fanStubs = [14, 22, 30, 38];
            var sparseGeometryCandidates = [];
            approachSpacings.forEach(function(approachSpacing) {
                fanStubs.forEach(function(fanStub) {
                    var plan = buildSparseLeaderPlan({
                        approachSpacing: approachSpacing,
                        fanStub: fanStub
                    });
                    sparseGeometryCandidates.push({
                        approachSpacing: approachSpacing,
                        fanStub: fanStub,
                        plan: plan,
                        metrics: measureSparseLeaderPlan(plan)
                    });
                });
            });
            sparseGeometryCandidates.sort(function(first, second) {
                if (first.metrics.crossingPairs !== second.metrics.crossingPairs) {
                    return first.metrics.crossingPairs - second.metrics.crossingPairs;
                }
                if (first.metrics.crowdedPairs !== second.metrics.crowdedPairs) {
                    return first.metrics.crowdedPairs - second.metrics.crowdedPairs;
                }
                if (Math.abs(first.metrics.crowding - second.metrics.crowding) > 0.25) {
                    return first.metrics.crowding - second.metrics.crowding;
                }
                if (Math.abs(first.metrics.minimumGap - second.metrics.minimumGap) > 0.25) {
                    return second.metrics.minimumGap - first.metrics.minimumGap;
                }
                if (Math.abs(first.metrics.length - second.metrics.length) > 0.5) {
                    return first.metrics.length - second.metrics.length;
                }
                return (first.approachSpacing + first.fanStub * 0.25) -
                    (second.approachSpacing + second.fanStub * 0.25);
            });
            var selectedSparseGeometry = sparseGeometryCandidates[0];
            var leaderRenderPlan = selectedSparseGeometry.plan;
            // Re-measure after selection so the per-route diagnostic fields
            // reflect only the chosen geometry, not the last search candidate.
            var selectedSparseMetrics = measureSparseLeaderPlan(leaderRenderPlan);
            var sparseLeaderCrowding = selectedSparseMetrics.crowding;
            var sparseCrowdedPairs = selectedSparseMetrics.crowdedPairs;
            var sparseMinimumGap = selectedSparseMetrics.minimumGap;
            chartNode.setAttribute(
                'data-map-leader-geometry-evaluations',
                String(sparseGeometryCandidates.length)
            );
            chartNode.setAttribute(
                'data-map-leader-approach-spacing',
                String(selectedSparseGeometry.approachSpacing)
            );
            chartNode.setAttribute(
                'data-map-leader-fan-stub',
                String(selectedSparseGeometry.fanStub)
            );
            chartNode.setAttribute(
                'data-map-leader-crossing-pairs',
                String(selectedSparseMetrics.crossingPairs)
            );
            chartNode.setAttribute(
                'data-map-leader-crowding-score',
                String(Math.round(sparseLeaderCrowding * 10) / 10)
            );
            chartNode.setAttribute('data-map-leader-crowded-pairs', String(sparseCrowdedPairs));
            chartNode.setAttribute(
                'data-map-leader-min-gap',
                Number.isFinite(sparseMinimumGap)
                    ? String(Math.round(sparseMinimumGap * 10) / 10)
                    : 'none'
            );
            ...
            leaderRenderPlan.forEach(function(plan) {
                ...
                lineLayer.appendChild(svgElement('path', {
                    ...
                    'data-route-crowding-score': String(Math.round((plan.crowdingScore || 0) * 10) / 10),
                    'data-route-crowded-with': plan.crowdedWith.join(',')
                }));
                if (anchorStyle === 'dot') {
