    ...
    function render(spec, chartNode) {
        ...
        function applyViewport() {
            if (mapPlan.viewportMode === 'data' && mapPlan.geoBounds) {
                chart.zoomToGeoBounds(mapPlan.geoBounds, 0);
            } else {
                chart.goHome(0);
            }
        }
        ...
        function layout() {
            ...
            routedEntries.forEach(function(entry) {
                ...
                var direction = entry.side === 'left' ? 1 : -1;
                var mapEdgeX = entry.side === 'left'
                    ? canvasRect.left - stageRect.left
                    : canvasRect.right - stageRect.left;
                var portX = mapEdgeX + direction * 42;
                var trackSpacing = entry.sideCount > 1 ? Math.min(6, 30 / (entry.sideCount - 1)) : 0;
                var trackX = mapEdgeX + direction * (8 + entry.laneIndex * trackSpacing);
                var pathData = [
                    'M', entry.point.x, entry.point.y,
                    'L', portX, entry.routeY,
                    'L', trackX, entry.routeY,
                    'L', trackX, endY,
                    'L', cardX, endY
                ].join(' ');
