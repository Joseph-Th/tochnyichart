(function(global) {
    ...
    function pack(entries, top, bottom, gap, ordering) {
        ...
        entries.sort(function(first, second) {
            if (ordering === 'optimized' || ordering === 'source') {
    ...
    function render(spec, chartNode) {
        ...
        chartNode.setAttribute('data-map-layout-stable', 'pending');
        ...
        function layout() {
            ...
            chartNode.setAttribute('data-map-callout-predicted-crossings', String(placement.predictedCrossings || 0));
            chartNode.setAttribute(
                'data-map-callout-predicted-crowding',
                String(Math.round((placement.predictedCrowding || 0) * 10) / 10)
            );
            chartNode.setAttribute('data-map-callout-side-switches', String(placement.sideSwitches || 0));
            chartNode.setAttribute('data-map-callout-assignment-evaluations', String(placement.assignmentEvaluations || 0));
            chartNode.setAttribute(
            ...
            pack(sides.left, topLeft, bottom, gap, 'optimized');
            pack(sides.right, topRight, bottom, gap, 'optimized');
            ...
            // Default leader geometry is deliberately straight. Placement has
            // already optimized card order; browser QA verifies that the final
            // rendered segments remain crossing-free.
            var directLeaderGeometry = { approachSpacing: 0, fanStub: 22 };
            var leaderRenderPlan = buildSparseLeaderPlan(directLeaderGeometry);
            ...
            chartNode.setAttribute('data-map-leader-geometry-evaluations', '1');
            chartNode.setAttribute('data-map-leader-approach-spacing', '0');
            chartNode.setAttribute('data-map-leader-fan-stub', String(directLeaderGeometry.fanStub));
            chartNode.setAttribute(
            ...
            leaderRenderPlan.forEach(function(plan) {
                ...
                lineLayer.appendChild(leaderNode);
                renderedDirectLeaderNodes.push(leaderNode);
            });
            chartNode.setAttribute(
                'data-map-leader-rendered-crossings',
                String(countRenderedLeaderCrossings(renderedDirectLeaderNodes))
            );
        }
        ...
        function settleLayout() {
            ...
            if (global.requestAnimationFrame) {
                global.requestAnimationFrame(function() {
                    projectionSizeKey = '';
                    layout();
                    chartNode.setAttribute('data-map-layout-stable', 'true');
                });
            } else {
                setTimeout(function() {
                    projectionSizeKey = '';
