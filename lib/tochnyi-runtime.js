...
(function(global) {
    ...
    function columnLabelInside(item, bounds, plan) {
        if (plan.labelMode === 'inside') return true;
        if (plan.labelMode === 'outside') return false;
        var span = Math.max(1, bounds.maximum - bounds.minimum);
        var normalized = (item.value - bounds.minimum) / span;
        var upperHeadroom = Math.max(0, 1 - normalized) * plan.chartHeight;
        var estimatedHeight = plan.compact ? 28 : 38;
        return upperHeadroom < estimatedHeight + 18;
    }
    ...
    function createScaffold(spec) {
        ...
        logo.src = assetsPrefix() + 'tochnyi-logo.png';
        logo.alt = 'Tochnyi';
        header.appendChild(logo);
        header.appendChild(element('div', 'tochnyi-date', 'Date: ' + spec.date));
        main.appendChild(header);

        main.appendChild(element('h1', 'tochnyi-title', spec.title));
        main.appendChild(element('p', 'tochnyi-subtitle', spec.subtitle));
        ...
        watermark.src = assetsPrefix() + 'watermark.svg';
        watermark.alt = '';
        ...
        if ([
            ...
            'composition.stacked', 'flow.waterfall', 'status.grid', 'story.sequence'
        ...
        var visibleFacts = compositionFacts(spec, scaffoldData, scaffoldTotal);
    ...
    function renderColumns(spec) {
        ...
        if (spec.options.showLabels) {
            series.bullets.push(function(rootRef, seriesRef, dataItem) {
                ...
                var inside = columnLabelInside(item, bounds, plan);
                return am5.Bullet.new(rootRef, {
                    locationY: positive ? 1 : 0,
                    sprite: am5.Label.new(rootRef, {
                        text: item.display,
                        fill: am5.color(Tochnyi.colors.black),
                        centerY: inside
                            ? (positive ? 0 : am5.percent(100))
                            : (positive ? am5.percent(100) : 0),
                        centerX: am5.percent(50),
                        dy: inside ? (positive ? 10 : -10) : (positive ? -10 : 10),
                        fontSize: plan.compact ? 16 : data.length > 4 ? 20 : 28,
                        fontWeight: '700',
                        fontFamily: Tochnyi.font.family,
                        oversizedBehavior: 'wrap',
