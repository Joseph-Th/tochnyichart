...
(function(global) {
    ...
    function createScaffold(spec) {
        ...
        var main = element('main', 'tochnyi-chart tochnyi-v2 frame-' + spec.narrative.frame + ' ' + recipeClass);
    ...
    function renderColumns(spec) {
        ...
        var chart = root.container.children.push(am5xy.XYChart.new(root, {
            ...
            paddingLeft: 10,
            paddingRight: 20,
            paddingTop: spec.emphasis ? 48 : 20,
            paddingBottom: 10
        }));
        ...
        var xRenderer = Tochnyi.createXRenderer(root, {
            ...
            fontSize: data.length > 4 ? 16 : 19,
            fontWeight: '600'
        });
        xRenderer.labels.template.setAll({
            oversizedBehavior: 'wrap',
            maxWidth: Math.max(145, Math.floor(850 / data.length)),
            textAlign: 'center',
            paddingTop: 10
        });
    ...
    function renderRanking(spec) {
        var data = preparedData(spec);
        if (spec.options.sort === 'none') data.sort(function(a, b) { return a.value - b.value; });
        var root = Tochnyi.createRoot('chartdiv');
        var chart = root.container.children.push(am5xy.XYChart.new(root, {
            panX: false,
            panY: false,
        ...
        xRenderer.grid.template.setAll({ strokeOpacity: 0.14 });
