...
(function(global) {
    ...
    function createScaffold(spec) {
        ...
        if (visibleFacts.length) {
            ...
            if (contextPlan.compactFacts) grid.classList.add('compact');
    ...
    function renderTrend(spec) {
        ...
        data = data.map(function(item, index) {
            return Object.assign({}, item, { trendLabel: labelPlan[index] });
        });
        var root = Tochnyi.createRoot('chartdiv');
        var chart = root.container.children.push(am5xy.XYChart.new(root, {
            panX: data.length > 10,
    ...
    function renderStatusGrid(spec, chartNode) {
        ...
        spec.data.forEach(function(item) {
            ...
            var titleWrap = element('div', 'tochnyi-status-title');
            if (item.icon) titleWrap.appendChild(semanticIcon(item.icon, 'tochnyi-status-icon'));
            titleWrap.appendChild(element('strong', '', item.label));
    ...
    function renderStoryFacets(spec, chartNode) {
        chartNode.classList.add('tochnyi-facets-stage');
        var groups = [];
        var byGroup = {};
        ...
        groups.forEach(function(groupName) {
            var section = element('section', 'tochnyi-facet-group');
            if (groupName) section.appendChild(element('h3', 'tochnyi-facet-group-title', groupName));
            var grid = element('div', 'tochnyi-facet-grid');
            byGroup[groupName].forEach(function(item) {
                var card = element('article', 'tochnyi-facet-card');
                card.setAttribute('data-tone', item.tone || 'neutral');
                if (item.status) card.setAttribute('data-status', item.status);
                var header = element('div', 'tochnyi-facet-header');
                header.appendChild(semanticIcon(item.icon || 'document', 'tochnyi-facet-icon'));
                var copy = element('div', 'tochnyi-facet-copy');
                copy.appendChild(element('strong', 'tochnyi-facet-label', item.label));
                copy.appendChild(element('div', 'tochnyi-facet-value', item.display || formatValue(item, spec)));
                header.appendChild(copy);
                if (item.direction) {
                    var marker = item.direction === 'up' ? '▲' : item.direction === 'down' ? '▼' : '•';
                    header.appendChild(element('span', 'tochnyi-facet-direction ' + item.direction, marker));
                }
                card.appendChild(header);
                card.appendChild(element('p', '', item.detail));
                grid.appendChild(card);
    ...
    function render(spec) {
        ...
        switch (spec.recipe) {
            ...
            case 'status.grid':
                ...
                break;
            case 'story.facets':
                renderStoryFacets(spec, chartNode);
                break;
            case 'map.regional':
