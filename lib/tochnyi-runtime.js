...
(function(global) {
    ...
    function categoricalColor(index) {
        var palette = Tochnyi.categoricalPalette || Tochnyi.palette;
        if (index < palette.length) return palette[index];
        var offset = index - palette.length;
        // Continue large category domains with a golden-angle sequence instead
        // of repeating a short set of brand shades.
        var hue = (24 + offset * 137.508) % 360;
        var saturation = 56 + (offset % 4) * 5;
        var lightness = 43 + (Math.floor(offset / 4) % 4) * 4;
    ...
    function rankingData(spec) {
        ...
        return {
            ...
            data: data.map(function(item, index) {
                ...
                if (plan.colorPolicy === 'categorical') {
                    color = categoricalColor(index);
                } else if (plan.colorPolicy === 'focus') {
                    if (index === 0) {
    ...
    function estimatedInlineLabelWidth(text, fontSize) {
        var value = String(text || '');
        var size = Number(fontSize) || 17;
        // The renderer cannot depend on a browser-font measurement being ready
        // before chart construction. This deliberately conservative estimate
        // reserves enough space for bold tabular-numeric labels such as
        // "105 stations" instead of letting the plot clip the trailing unit.
        return Math.ceil(value.length * size * 0.64);
    }

    function rankingValueLabelGutter(data, plan) {
        var maximumLabelWidth = Math.max.apply(null, data.map(function(item) {
            return estimatedInlineLabelWidth(item.display, data.length > 24 ? 13 : 17);
        }).concat([0]));
        if (plan.labelMode === 'inside') {
            return { gutter: 24, maximumLabelWidth: maximumLabelWidth };
        }
        var minimumGutter = plan.labelMode === 'outside' ? 95 : 58;
        return {
            gutter: Math.max(minimumGutter, maximumLabelWidth + 34),
    ...
    function addValueReferences(root, axis, spec, orientation) {
        (spec.references || []).forEach(function(reference) {
            ...
            range.get('label').setAll({
                ...
                maxWidth: 140,
                // Reference labels belong in the plot, not on top of ordinary
                // axis tick labels. Horizontal reference lines therefore get a
                // larger inward x-offset; vertical references only need a small
                // lateral offset from their line.
                dx: orientation === 'vertical' ? 24 : 170,
                dy: orientation === 'vertical' ? -20 : -12,
                paddingTop: 4,
                paddingRight: 6,
                paddingBottom: 4,
                paddingLeft: 6,
    ...
    function renderColumns(spec) {
        ...
        addValueReferences(root, yAxis, spec, 'horizontal');
        ...
        if (spec.options.showLabels) {
            var columnLabelFontSize = plan.compact ? 16 : data.length > 4 ? 20 : 28;
            var columnLabelMaxWidth = plan.compact ? 90 : 180;
            var columnLabelMode = global.TochnyiVisualPlan && global.TochnyiVisualPlan.columnLabelGroupMode
                ? global.TochnyiVisualPlan.columnLabelGroupMode(data, bounds, plan, {
                    fontSize: columnLabelFontSize,
                    labelMaxWidth: columnLabelMaxWidth,
                    plotHeight: Math.max(220, plan.chartHeight - 110)
                })
                : plan.labelMode;
            // Two-component decompositions now require an independent scale
            // anchor in addition to the reconciled total. Multiple horizontal
            // reference lines can occupy the same headroom as outside column
            // labels, so keep the component values inside their bars and leave
            // the reference band visually clear.
            if (spec.recipe === 'composition.components' && (spec.references || []).length > 1) {
                columnLabelMode = 'inside';
            }
            var columnLabelPlan = Object.assign({}, plan, { labelMode: columnLabelMode });
            if (chartHost) chartHost.setAttribute('data-column-label-mode', columnLabelMode);
            series.bullets.push(function(rootRef, seriesRef, dataItem) {
                var item = dataItem.dataContext;
                var fontSize = columnLabelFontSize;
                var placement = columnLabelPlacement(item, bounds, columnLabelPlan, { fontSize: fontSize });
                ...
                function applyMeasuredPlacement() {
                    ...
                    var measured = columnLabelPlacement(item, bounds, columnLabelPlan, {
                        fontSize: fontSize,
    ...
    function stackedTrendLegendPlan(categoryCount, viewportWidth, compact) {
        var preferredColumns = viewportWidth <= 520 ? 3 : viewportWidth <= 860 ? 4 : 5;
        var columns = Math.max(1, Math.min(categoryCount, preferredColumns));
        var rows = Math.max(1, Math.ceil(categoryCount / columns));
        // Compact legends often wrap long category names onto a second line.
        // Reserve for that wrapped line instead of estimating from the marker
        // height alone; otherwise the legend can visually spill into the plot.
        var rowHeight = compact ? 34 : 29;
        var gap = compact ? 14 : 14;
        return {
            columns: columns,
            rows: rows,
            gap: gap,
            bandHeight: rows * rowHeight + gap + 8
        };
    }

    function renderStackedTrend(spec) {
        var periods = spec.data || [];
        var categories = periods[0].segments.map(function(segment) { return segment.label; });
        var rows = periods.map(function(item) {
            var row = {
                label: item.label,
                period: item.period,
                total: 0
            ...
            item.segments.forEach(function(segment, index) {
                row['segment' + index] = segment.value;
                row.total += segment.value;
            });
            row.totalDisplay = formatRawValue(row.total, spec);
            return row;
        });
        var totalData = rows.map(function(row) {
            return { label: row.label, value: row.total, display: row.totalDisplay };
        });
        var plan = visualPlan(spec, totalData);
        var compactTotalLabels = plan.viewportWidth <= 1100;
        rows.forEach(function(row) {
            var measure = spec.measure || {};
            var compactSuffix = measure.unit === '%'
                ? '%'
                : measure.suffix !== undefined && !/^\s/.test(String(measure.suffix))
                    ? String(measure.suffix)
                    : '';
            row.totalLabel = compactTotalLabels
                ? (measure.prefix || '') + formatNumber(row.total, measure.decimals || 0) + compactSuffix
                : row.totalDisplay;
        });
        var root = Tochnyi.createRoot('chartdiv');
        root.container.set('layout', root.verticalLayout);
        var legendPlan = stackedTrendLegendPlan(categories.length, plan.viewportWidth, plan.compact);
        var chartHost = document.getElementById('chartdiv');
        if (chartHost && spec.options.showLegend) {
            chartHost.setAttribute('data-stacked-trend-legend-band', String(legendPlan.bandHeight));
            chartHost.setAttribute('data-stacked-trend-legend-rows', String(legendPlan.rows));
        }

        var legend = null;
        if (spec.options.showLegend) {
            var legendBand = root.container.children.push(am5.Container.new(root, {
                width: am5.percent(100),
                height: legendPlan.bandHeight,
                layout: root.verticalLayout,
                paddingBottom: legendPlan.gap
            }));
            legend = legendBand.children.push(am5.Legend.new(root, {
                width: am5.percent(100),
                centerX: am5.percent(50),
                x: am5.percent(50),
                layout: am5.GridLayout.new(root, {
                    maxColumns: legendPlan.columns,
                    fixedWidthGrid: true
                }),
        ...
        var chart = root.container.children.push(am5xy.XYChart.new(root, {
            ...
            paddingLeft: 10,
            paddingRight: 22,
            paddingTop: spec.options.showLegend ? 18 : 8,
            paddingBottom: 10
        }));

        var xRenderer = Tochnyi.createXRenderer(root, {
            minGridDistance: plan.compact ? 50 : plan.viewportWidth <= 800 ? 26 : 42,
            cellStartLocation: 0.12,
            cellEndLocation: 0.88,
            fontSize: plan.compact ? 12 : 15,
            fontWeight: '600'
        });
        xRenderer.labels.template.setAll({
            oversizedBehavior: 'truncate',
            maxWidth: plan.viewportWidth <= 800 ? 36 : 88,
            textAlign: 'center',
            paddingTop: 9,
            dx: plan.viewportWidth <= 800 ? 10 : 0
        });
        if (plan.viewportWidth <= 800) {
            xRenderer.labels.template.adapters.add('text', function(text) {
                return String(text || '').replace(/^([A-Z][a-z]{2}) (\d{2})$/, '$1\n$2');
            });
        }
        var xAxis = chart.xAxes.push(am5xy.CategoryAxis.new(root, {
            categoryField: 'label',
        ...
        var bounds = axisBounds(spec, totalData);
        var yRenderer = Tochnyi.createYRenderer(root, { fontSize: plan.compact ? 12 : 15 });
        yRenderer.grid.template.setAll({ strokeOpacity: 0.14 });
        var yAxis = chart.yAxes.push(am5xy.ValueAxis.new(root, {
            min: bounds.minimum,
        ...
        var seriesList = categories.map(function(category, index) {
            var color = categoricalColor(index);
            var series = chart.series.push(am5xy.ColumnSeries.new(root, {
                name: category,
                xAxis: xAxis,
                yAxis: yAxis,
                valueYField: 'segment' + index,
                categoryXField: 'label',
                stacked: true,
                tooltip: am5.Tooltip.new(root, {
                    labelText: category + '\n{label}: {valueY.formatNumber("#,###")}'
                })
            }));
            series.columns.template.setAll({
                ...
                fillOpacity: Tochnyi.marks.column.fillOpacity,
                strokeOpacity: Tochnyi.marks.column.strokeOpacity,
                strokeWidth: Tochnyi.marks.column.strokeWidth
            });
            Tochnyi.applyColumnAppearance(series);
        ...
        if (legend) legend.data.setAll(seriesList);

        if (spec.options.showLabels) {
            var totalSeries = chart.series.push(am5xy.LineSeries.new(root, {
                xAxis: xAxis,
                yAxis: yAxis,
                valueYField: 'total',
                categoryXField: 'label'
            }));
            totalSeries.strokes.template.setAll({ strokeOpacity: 0 });
            totalSeries.bullets.push(function(rootRef, seriesRef, dataItem) {
                var item = dataItem.dataContext;
                var label = am5.Label.new(rootRef, {
                    ...
                    dy: -8,
                    fontFamily: Tochnyi.font.family,
                    fontSize: plan.compact ? 11 : plan.viewportWidth <= 1280 ? 13 : 14,
                    fontWeight: '700',
                    fill: am5.color(Tochnyi.colors.black),
                    textAlign: 'center',
                    paddingLeft: 0,
            ...
            totalSeries.data.setAll(rows);
            totalSeries.appear(0, 0);
        }

        var staticMode = new URLSearchParams(global.location.search).has('static');
        seriesList.forEach(function(series, index) {
            if (index === 0) return;
            var duration = staticMode || spec.options.animate === false ? 0 : 800;
            var delay = staticMode || spec.options.animate === false ? 0 : 80;
            series.appear(duration, delay);
        });
        animate(chart, seriesList[0], spec);
    }
    ...
    function renderRanking(spec) {
        ...
        var valueLabelSpace = rankingValueLabelGutter(data, plan);
        var root = Tochnyi.createRoot('chartdiv');
        var chartHost = document.getElementById('chartdiv');
        if (chartHost) {
            chartHost.setAttribute('data-ranking-value-label-gutter', String(Math.round(valueLabelSpace.gutter)));
            chartHost.setAttribute('data-ranking-value-label-estimated-width', String(Math.round(valueLabelSpace.maximumLabelWidth)));
        }
        var chart = root.container.children.push(am5xy.XYChart.new(root, {
            panX: false,
            panY: false,
            ...
            maskBullets: false,
            paddingLeft: 10,
            paddingRight: valueLabelSpace.gutter,
            paddingTop: plan.density === 'minimal' ? 8 : 18,
            paddingBottom: 10
        }));

        var yRenderer = am5xy.AxisRendererY.new(root, {
            minGridDistance: data.length > 24 ? 20 : 28,
            inversed: true,
            cellStartLocation: 0.14,
            cellEndLocation: 0.86
        });
        yRenderer.labels.template.setAll({
            fontSize: data.length > 50 ? 12 : data.length > 24 ? 13 : data.length > 8 ? 15 : 18,
            fontFamily: Tochnyi.font.family,
            fontWeight: '600',
            oversizedBehavior: 'wrap',
        ...
        addValueReferences(root, xAxis, spec, 'vertical');
        ...
        series.columns.template.setAll({
            height: am5.percent(data.length > 24 ? 62 : 68),
            cornerRadiusTR: 6,
            cornerRadiusBR: 6
        });
        applySemanticColumnAppearance(series);
    ...
    function renderBenchmarkGap(spec, chartNode) {
        ...
        svg.appendChild(svgElement('text', {
            x: left, y: 24, class: 'tochnyi-svg-axis-title',
            'data-tochnyi-reserved': 'benchmark-gap-key'
        }, 'ACTUAL VS BENCHMARK'));
        ...
        data.forEach(function(item, index) {
            ...
            var actualColor = TONE_HEX.primary;
            var gapColor = tone === 'primary'
                ? '#78aee3'
                : tone === 'neutral'
                    ? '#aab4bd'
            ...
            var benchmarkText = item.benchmarkDisplayValue || ('Benchmark ' + formatRawValue(benchmark, spec));
    ...
    function renderDumbbell(spec, chartNode) {
        ...
        svg.appendChild(svgElement('circle', {
            cx: right - 75, cy: 24, r: 8, fill: TONE_HEX.primary,
