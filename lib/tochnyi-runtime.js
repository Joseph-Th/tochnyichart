...
(function(global) {
    ...
    function renderBenchmarkGap(spec, chartNode) {
        ...
        var width = 1000;
        var left = single ? 205 : 240;
        var right = 930;
        var top = single ? 104 : 72;
        var rowHeight = single ? 104 : 82;
        var height = single ? 252 : top + data.length * rowHeight + 50;
        ...
        data.forEach(function(item, index) {
            ...
            var gapColor = TONE_HEX[tone] || TONE_HEX.warning;
            ...
            var gapWidth = Math.abs(highX - lowX);
            var gapInside = gapWidth >= 115;
            var gapLabelX = gapInside ? (lowX + highX) / 2 : (highX > right - 150 ? lowX - 10 : highX + 10);
            var gapLabelAnchor = gapInside ? 'middle' : (highX > right - 150 ? 'end' : 'start');
            ...
            svg.appendChild(svgElement('text', {
                x: actualX, y: y - 23, 'text-anchor': actualX > right - 130 ? 'end' : 'middle',
                class: 'tochnyi-svg-value', 'data-tochnyi-reserved': 'actual-label', 'data-label-group': group
            }, item.display || formatRawValue(actual, spec)));
            svg.appendChild(svgElement('text', {
                x: benchmarkX, y: y + 40, 'text-anchor': benchmarkX > right - 120 ? 'end' : 'middle',
                class: 'tochnyi-svg-benchmark-label', 'data-tochnyi-reserved': 'benchmark-label', 'data-label-group': group
            }, item.benchmarkDisplayValue || ('Benchmark ' + formatRawValue(benchmark, spec))));
            svg.appendChild(svgElement('text', {
                x: gapLabelX, y: y + 5, 'text-anchor': gapLabelAnchor,
                class: 'tochnyi-svg-gap-label', fill: gapInside ? '#ffffff' : gapColor,
    ...
    function renderConvergingSignals(spec, chartNode) {
        ...
        if (compact) {
            connectorGroup.appendChild(svgElement('path', {
                d: 'M 300 160 C 300 245 260 330 286 399',
                class: 'tochnyi-signal-link'
            }));
            connectorGroup.appendChild(svgElement('path', {
                d: 'M 300 365 C 300 385 340 392 314 409',
                class: 'tochnyi-signal-link'
            }));
            connectorGroup.appendChild(svgElement('path', {
                d: 'M 300 462 C 300 480 300 492 300 505',
                class: 'tochnyi-signal-link outcome'
            }));
        } else {
            connectorGroup.appendChild(svgElement('path', {
                d: 'M 444 112 C 500 112 520 174 552 194',
                class: 'tochnyi-signal-link'
            }));
            connectorGroup.appendChild(svgElement('path', {
                d: 'M 444 302 C 500 302 520 240 552 220',
                class: 'tochnyi-signal-link'
            }));
            connectorGroup.appendChild(svgElement('path', {
                d: 'M 598 207 C 632 207 655 207 690 207',
                class: 'tochnyi-signal-link outcome'
            }));
        }
        ...
        svg.appendChild(svgElement('circle', {
            ...
            cy: hub.y,
            r: 25,
            class: 'tochnyi-signal-hub'
        }));
        var operatorGroup = svgElement('g', { class: 'tochnyi-signal-operator', 'aria-hidden': 'true' });
        if (relationship.operator === 'multiply') {
            operatorGroup.appendChild(svgElement('line', { x1: hub.x - 8, y1: hub.y - 8, x2: hub.x + 8, y2: hub.y + 8 }));
            operatorGroup.appendChild(svgElement('line', { x1: hub.x + 8, y1: hub.y - 8, x2: hub.x - 8, y2: hub.y + 8 }));
        } else {
            operatorGroup.appendChild(svgElement('line', { x1: hub.x - 9, y1: hub.y, x2: hub.x + 9, y2: hub.y }));
            operatorGroup.appendChild(svgElement('line', { x1: hub.x, y1: hub.y - 9, x2: hub.x, y2: hub.y + 9 }));
        }
        svg.appendChild(operatorGroup);
        chartNode.appendChild(svg);
    }

    function renderPictogramComparison(spec, chartNode) {
        chartNode.classList.add('tochnyi-count-pictogram-stage');
        var data = preparedData(spec);
        var maximum = Math.max.apply(null, data.map(function(item) { return Number(item.value) || 0; }).concat([1]));
        var columns = Number(spec.visual && spec.visual.columns) || (maximum <= 100 ? 10 : maximum <= 225 ? 15 : 20);
        var comparison = element('div', 'tochnyi-count-comparison');
        comparison.style.setProperty('--tochnyi-count-columns', String(columns));

        data.forEach(function(item, index) {
            var count = Math.max(0, Number(item.value) || 0);
            var tone = item.tone || ['primary', 'critical', 'secondary', 'warning'][index] || 'neutral';
            var card = element('article', 'tochnyi-count-card');
            card.setAttribute('data-tone', tone);
            var header = element('header', 'tochnyi-count-header');
            header.appendChild(element('strong', 'tochnyi-count-value', item.display || formatRawValue(count, spec)));
            header.appendChild(element('span', 'tochnyi-count-label', item.label));
            card.appendChild(header);

            var symbols = element('div', 'tochnyi-count-symbols');
            symbols.setAttribute('role', 'img');
            symbols.setAttribute('aria-label', item.label + ': ' + count + ' ' + (spec.measure.unit || 'units'));
            var iconName = item.icon || (spec.visual && spec.visual.icon) || 'dot';
            for (var unitIndex = 0; unitIndex < count; unitIndex += 1) {
                var symbol = iconName === 'dot'
                    ? element('span', 'tochnyi-count-dot')
                    : semanticIcon(iconName, 'tochnyi-count-icon');
                symbol.setAttribute('data-tochnyi-mark', 'count-unit');
                symbol.setAttribute('data-label-group', 'count-item-' + index);
                symbol.setAttribute('aria-hidden', 'true');
                symbols.appendChild(symbol);
            }
            if (count === 0) symbols.appendChild(element('span', 'tochnyi-count-zero', '0'));
            card.appendChild(symbols);
            comparison.appendChild(card);
        });
        chartNode.appendChild(comparison);
    }
    ...
    function render(spec) {
        ...
        switch (spec.recipe) {
            ...
            case 'comparison.pictogram':
                renderPictogramComparison(spec, chartNode);
                break;
