(function(root, factory) {
    ...
    function minimumChartHeight(spec, count, viewportWidth) {
        ...
        if (spec.recipe === 'comparison.pictogram') return viewportWidth <= 760 ? 470 : 400;
    ...
    function canShrinkChart(spec) {
        return [
            ...
            'comparison.range', 'comparison.pictogram', 'comparison.benchmark-gap', 'comparison.dumbbell', 'relationship.converging-signals', 'trend.line', 'timeline.duration', 'composition.donut',
    ...
    function defaultHeight(spec, count, viewportWidth) {
        ...
        if (spec.recipe === 'comparison.pictogram') {
            var maximumCount = Math.max.apply(null, (spec.data || []).map(function(item) { return Number(item.value) || 0; }).concat([1]));
            var pictogramRows = Math.ceil(maximumCount / 20);
            return clamp(310 + pictogramRows * 22 + densityAdjustment(density), 470, 760);
        }
