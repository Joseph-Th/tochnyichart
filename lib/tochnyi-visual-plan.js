(function(root, factory) {
    ...
    function rankingHeight(count, density) {
        if (count <= 12) return clamp(140 + count * 56 + densityAdjustment(density), 340, 700);
        var rowHeight = count > 60 ? 36 : count > 30 ? 38 : 42;
        return 180 + count * rowHeight + densityAdjustment(density);
    }
    ...
    function minimumChartHeight(spec, count, viewportWidth) {
        if (spec.recipe === 'ranking.horizontal') {
            if (count > 12) return 150 + count * 32;
            return clamp(180 + count * 42, 320, 620);
        }
        ...
        if (spec.recipe === 'relationship.converging-signals') return viewportWidth <= 900 ? 650 : 390;
        if (spec.recipe === 'trend.line') return 360;
        if (spec.recipe === 'trend.stacked') return 430;
        if (spec.recipe === 'timeline.duration') return 330;
        if (spec.recipe === 'composition.donut') return 360;
        if (spec.recipe === 'composition.components') return 350;
    ...
    function canShrinkChart(spec, count) {
        if (spec.recipe === 'ranking.horizontal' && count > 12) return false;
        return [
            'comparison.change', 'comparison.scenarios', 'comparison.diverging',
            'comparison.range', 'comparison.benchmark-gap', 'comparison.dumbbell', 'relationship.converging-signals', 'trend.line', 'trend.stacked', 'timeline.duration', 'composition.donut', 'composition.components',
    ...
    function defaultHeight(spec, count, viewportWidth) {
        ...
        if (spec.recipe === 'trend.stacked') return viewportWidth <= 600 ? 650 : density === 'detailed' ? 650 : 590;
        ...
        if (spec.recipe === 'relationship.converging-signals') {
            return viewportWidth <= 900 ? 720 + densityAdjustment(density) : 500 + densityAdjustment(density);
        }
    ...
    function rankingPolicy(spec, data) {
        ...
        if (emphasis !== 'ranking' || data.length < 2) {
            return { colorPolicy: 'categorical', accentSecond: false };
        }
    ...
    function columnLabelFitProfile(item, bounds, plan, metrics) {
        var options = metrics || {};
        ...
        var labelMaxWidth = Number(options.labelMaxWidth) || (plan && plan.compact ? 90 : 180);
        var displayLength = String(item && item.display || '').length;
        var estimatedTextWidth = Math.max(fontSize * 1.5, displayLength * fontSize * 0.66);
        var estimatedLines = Math.max(1, Math.ceil(estimatedTextWidth / Math.max(40, labelMaxWidth)));
        var labelHeight = Number(options.labelHeight) || Math.ceil(fontSize * 1.82 * estimatedLines);
    ...
    function columnLabelGroupMode(items, bounds, plan, metrics) {
        var mode = plan && plan.labelMode || 'auto';
        if (mode !== 'auto') return mode;
        var data = Array.isArray(items) ? items : [];
        if (data.length < 2) return 'auto';

        var profiles = data.map(function(item) {
            return columnLabelFitProfile(item, bounds, plan, metrics);
        });
        var allEndOutside = profiles.every(function(profile) { return profile.outsideFits; });
        var allInside = profiles.every(function(profile) { return profile.insideFits; });

        // Prefer one label treatment for the whole bar family. Independent
        // per-bar decisions make near-equal bars flip between inside and
        // outside merely because one endpoint has a few extra pixels of
        // headroom. Mixed placement is reserved for a genuine fit conflict.
        if (allEndOutside) return 'outside';
        if (allInside) return 'inside';

        var forcedInside = profiles.some(function(profile) {
            return profile.insideFits && !profile.outsideFits;
        });
        var forcedOutside = profiles.some(function(profile) {
            return !profile.insideFits && (profile.outsideFits || profile.startOutsideFits);
        });
        if (forcedInside && !forcedOutside) return 'inside';
        if (forcedOutside && !forcedInside) return 'outside';
        return 'auto';
    }

    function columnLabelPlacement(item, bounds, plan, metrics) {
        var profile = columnLabelFitProfile(item, bounds, plan, metrics);
        var positive = profile.positive;
        var insideFits = profile.insideFits;
        var outsideFits = profile.outsideFits;
        var startOutsideFits = profile.startOutsideFits;
        var barHeight = profile.barHeight;
        var labelHeight = profile.labelHeight;
        var outsideGap = profile.outsideGap;
        var mode = plan && plan.labelMode || 'auto';
        var placement = 'end';
    ...
    function benchmarkGapLabelPlan(labels, anchors, bounds, areas) {
        var entries = Array.isArray(labels) ? labels : [];
        var points = Array.isArray(anchors) ? anchors : [];
        var regions = Array.isArray(areas) ? areas : [];
        var left = Number(bounds && bounds.left) || 0;
        var right = Number(bounds && bounds.right) || 1000;
        var laneGap = 12;
        var lanes = [[], [], []];

        function estimatedWidth(text) {
            return clamp(String(text || '').length * 7.2 + 16, 44, 220);
        }

        return entries.map(function(text, index) {
            var width = estimatedWidth(text);
            var region = regions[index] || null;
            if (region && Number.isFinite(Number(region.left)) && Number.isFinite(Number(region.right))) {
                var regionLeft = Math.min(Number(region.left), Number(region.right));
                var regionRight = Math.max(Number(region.left), Number(region.right));
                var regionWidth = regionRight - regionLeft;
                if (regionWidth >= width + 18) {
                    return {
                        x: (regionLeft + regionRight) / 2,
                        lane: 0,
                        width: width,
                        textAnchor: 'middle',
                        placement: 'inside',
                        role: region.role || 'segment'
                    };
                }
            }
            if (region && region.role === 'benchmark') {
                var markerX = clamp(Number(points[index]), left + width / 2, right - width / 2);
                return {
                    x: markerX,
                    lane: 0,
                    width: width,
            ...
            var requestedX = Number(points[index]);
            if (!Number.isFinite(requestedX)) requestedX = (left + right) / 2;
            var x = clamp(requestedX, left + width / 2, right - width / 2);
            var interval = { left: x - width / 2, right: x + width / 2 };
            var lane = 0;
            for (; lane < lanes.length; lane += 1) {
                var collides = lanes[lane].some(function(placed) {
                    return interval.left < placed.right + laneGap && interval.right > placed.left - laneGap;
                });
                if (!collides) break;
            }
    ...
    function resolveVisualPlan(spec, data, viewportWidth) {
        ...
        return {
            ...
            canShrinkChart: canShrinkChart(spec, items.length),
            labelMode: labelMode,
            showAxisTitle: density !== 'minimal' && width > 600,
            showGrid: density !== 'minimal',
            categoryLabelWidth: width <= 760 ? 190 : items.length > 30 ? 275 : items.length > 8 ? 250 : 300,
    ...
    return {
        ...
        mapHeight: mapHeight,
        columnLabelGroupMode: columnLabelGroupMode,
        columnLabelPlacement: columnLabelPlacement,
        benchmarkGapLabelPlan: benchmarkGapLabelPlan,
