...
const SOURCE_ENRICHMENT_POLICY = Object.freeze({
  ...
  basisRule: 'For rates and shares, identify the numerator and denominator or the total and affected population. When those amounts are reported or retrievable, derive and plot the tangible amounts with level geometry; keep the percentage in labels, emphasis, or annotations. The denominator or population must also appear on the plotted scale as a point, reference, benchmark, or complete composition. A basis rail alone is not sufficient primary evidence.',
  benchmarkGapRule: 'For prices, costs, freight, margins, discounts, premiums, shortfalls, and overages, research the underlying benchmark total and actual amount. In comparison.benchmark-gap, value is the tangible actual level and benchmark is the total reference level; the gap amount belongs in gapDisplayValue. Prefer one segmented row when that relationship fully carries the story. Do not add a second row that merely restates the benchmark or the mathematically implied remainder.',
  relationshipRule: 'When two drivers and one outcome use different real-world quantities, do not force them onto a shared percentage axis or plot one side while relegating the other to supporting facts. Use relationship.converging-signals for claims such as fewer purchases plus higher prices producing higher spending. Each factor and the outcome is drawn as an independent local quantitative signal connected through one merge point. Connector width never encodes magnitude. Use identity mode only when scope and period reconcile exactly; otherwise use directional mode and disclose the mismatch in note.',
  ...
  evidenceUtilizationRule: 'Any comparable datapoint that materially defines the title must be primary geometry, not a supporting fact. Three or more ordered observations establishing slowdown, acceleration, reversal, or persistence require a trend. Supporting facts are only for evidence that is genuinely secondary to the plotted claim.',
  coverageRule: 'When a shortage, import, reserve, or shipment story is interpreted as days of consumption, demand coverage, or share of need, place that denominator on the visual scale. Convert amounts to coverage time when defensible or add a visible consumption or demand benchmark in the same unit.',
...
const VISUAL_EVIDENCE_CONTRACT = Object.freeze({
  ...
  compositionRule: 'A composition must lead with proportional marks and one label treatment per segment. Do not repeat the same percentage, amount, and category both inside the bar and immediately below it. It cannot collapse into a primaryMetric or use supporting facts to restate segment values.',
...
const STANDARD_SELECTION_RULES = Object.freeze([
  ...
  Object.freeze({ when: 'Three to five actual, expected, prior, target, or alternative values for the same named quantity, scope, and period; two values only when a numeric reference, basis, mechanism, consequence, denominator, or comparison fact makes the chart independently useful', use: 'comparison.scenarios', example: 'specs/examples/central-bank-scenarios.json' }),
  ...
  Object.freeze({ when: 'One or more independent actual values sit inside benchmark totals; one segmented row is preferred when one relationship fully carries the story', use: 'comparison.benchmark-gap', example: 'specs/examples/urals-benchmark-gap.json' }),
  ...
  Object.freeze({ when: 'Two linked drivers and one outcome use different quantities or units and need independent local quantitative signals that visibly converge', use: 'relationship.converging-signals', example: 'specs/examples/converging-signals.json' }),
...
const COMPOSABLE_FEATURES = Object.freeze([
  ...
  Object.freeze({ need: 'An amount is meaningful only relative to daily consumption, monthly demand, or total need', add: 'convert to coverage time or add a visible reference in the same unit' }),
...
const SHARED_AUTHORING_RULES = Object.freeze([
  ...
  'When an amount is interpreted through daily consumption, monthly demand, coverage, or total need, visualize that denominator as a reference or convert the marks to coverage time.',
