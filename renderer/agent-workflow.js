...
const SOURCE_ENRICHMENT_POLICY = Object.freeze({
  ...
  coverageRule: 'When a shortage, import, reserve, or shipment story is interpreted as days of consumption, demand coverage, or share of need, place the demand denominator on the visual scale. If the input contains two or more physical-volume components, visualEvidenceAudit.coverageAudit must disposition every reported volume as a plotted component, denominator, or specifically excluded item. Plot all retained components and total need in one tangible unit; days of coverage may be secondary context, never a replacement for the decomposition.',
...
const STANDARD_SELECTION_RULES = Object.freeze([
  ...
  Object.freeze({ when: 'Two source-supported causal drivers and one different outcome measure three distinct quantities, with an explicit mechanism formula; never repeated prices, repeated volumes, or one measure at different dates', use: 'relationship.converging-signals', example: 'specs/examples/converging-signals.json' }),
